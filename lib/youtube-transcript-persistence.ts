...
export async function finalizeVideoIntakeRun(input: {
  ...
  const processLog = [
    ...
    {
      stage: "transcribe_and_review_videos",
      status: failures.length ? (ready ? "partial" : "blocked_for_detected_video") : deferredVideoIds.length ? "partial" : "complete",
      ready,
  ...
  const { error } = await client.from("research_runs").update({
    ...
    videos_found: input.results.length + deferredVideoIds.length,
    transcripts_ready: ready,
    warnings,
    process_log: processLog,
    summary: blocked
  ...
  const transcriptStatus = failures.length ? (ready ? "partial" : "blocked") : deferredVideoIds.length ? "partial" : "complete";
  const { error: slotError } = await client.from("research_slot_runs").update({
    ...
    live_publication_status: "not_required",
    hybrid_handoff_status: "not_required",
    videos_detected: input.results.length + deferredVideoIds.length,
    transcripts_saved: ready,
