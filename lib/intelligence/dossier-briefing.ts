...
function lessonFromSource(source: JourneyStorySource, number: number, story: EditionStory | undefined): DossierLesson {
...
function topicChips(stories: EditionStory[], changes: MaterialChange[]) {
  const changed = new Set(changes.map((change) => change.id));
  return unique(stories
    .filter((story) => changed.has(story.id))
...
export function composeDossierBriefing({
  ...
  const sourceIds = new Set(sources.map((source) => source.storyId));
  for (const change of changes) {
    if (!sourceIds.has(change.id)) warnings.push(`Dossier omitted Story ${change.id}: exact immutable Canonical Story Reasoning V1 snapshot is unavailable.`);
  }
  const lessons = sources.map((source, index) => lessonFromSource(source, index + 1, storyById.get(source.storyId)));
  ...
  return {
    ...
    generatedAt,
    opening: {
      headline: lead?.title || "No materially supported Story change in this edition.",
      summary: lead?.body[0] || "The canonical Live edition contains no material Story change to explain.",
      marketState: marketTape.regimeSummary,
      topicChips: topicChips(stories, changes),
    },
    quickSummary,
