 ...
 * Stories. Its structured output now includes a bounded Story-maintenance
...
export async function applyMarketBeliefStoryMaintenance(input: {
  ...
  if (!stage) return { evaluatedStoryIds: [], materiallyChangedStoryIds: [], reused: false, stageRunId: null };
  const assessments = Array.isArray(stage.output_payload?.storyAssessments) ? stage.output_payload.storyAssessments : [];
  if (!assessments.length) return { evaluatedStoryIds: [], materiallyChangedStoryIds: [], reused: false, stageRunId: stage.id };

  const [stories, evidenceRows] = await Promise.all([
    intelligenceRest<MaintenanceStory[]>(
      "stories?select=id,slug,title,thesis,status,confidence,market_question,dominant_narrative,strongest_support,strongest_contradiction,confirmation_trigger,invalidation_trigger,next_catalyst,assets&status=neq.archived&status=neq.discarded&order=updated_at.desc",
    ),
    intelligenceRest<EvidenceRow[]>(
      "intelligence_evidence?select=id,claim_text,summary,evidence_class,support_direction,event_at,published_at,affected_assets,affected_topics,provenance_urls,source:intelligence_evidence_sources(source_name,source_tier,reliability_score,ancestry_group_id)&freshness_status=neq.superseded&order=event_at.desc.nullslast,received_at.desc&limit=72",
    ),
  ]);
  const evidence = evidencePack(evidenceRows);
  const analysisAsOf = stage.completed_at || new Date().toISOString();
  const targets = await buildStoryReviewTargets({ stories, evidence, analysisAsOf });
