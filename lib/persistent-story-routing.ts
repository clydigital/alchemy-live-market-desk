...
const ACTIVE_STORY_STATUSES = ["publish", "develop", "monitor"];
...
function storyScore(item: IntakeItemInput, story: RoutableStory, now: Date) {
...
export function matchFreshNewsToStories(item: IntakeItemInput, stories: RoutableStory[], now = new Date()) {
  if (item.itemType !== "news" || item.itemKey.startsWith("calendar:")) return item.affectedStorySlugs ?? [];
  const scored = stories
    .map((story) => ({ story, score: storyScore(item, story, now) }))
...
export async function routeFreshNewsToPersistentStories(items: IntakeItemInput[], now = new Date()) {
  if (!items.some((item) => item.itemType === "news" && !item.itemKey.startsWith("calendar:"))) return items;
  const client = createSupabaseAdminClient();
  const { data, error } = await client
    .from("stories")
    .select("slug,title,thesis,status,confidence,assets")
    .in("status", ACTIVE_STORY_STATUSES);
  if (error) throw new Error(`Persistent Story routing failed: ${error.message}`);
  const stories = (data ?? []) as RoutableStory[];
  if (!stories.length) return items;
  return items.map((item) => ({
    ...item,
    affectedStorySlugs: matchFreshNewsToStories(item, stories, now),
  }));
}
