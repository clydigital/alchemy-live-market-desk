    ...
    <main className="site-stage">
      <section className="workspace-shell">
        ...
        {activeTab === "Overview" && (
          <div className="overview-grid">
            <article className="panel snapshot-panel">
              ...
              <svg className="mini-chart" viewBox="0 0 700 190" preserveAspectRatio="none" aria-label="Live S&P 500 proxy momentum from Nasdaq data">
              ...
              <div className="chart-dates"><span>Start</span><span>{pulsePoints.length} sessions</span><a href={spxSeries?.sourceUrl} target="_blank" rel="noreferrer">Open source ↗</a></div>
            </article>
            ...
            <article className="panel trend-panel">
              ...
              <div className="trend-legend"><span>Indexed performance</span><i className="purple-dot"/>S&P 500<i className="blue-dot"/>Equal weight <a href={spxSeries?.sourceUrl} target="_blank" rel="noreferrer">Source ↗</a></div>
              <svg className="trend-chart" viewBox="0 0 920 300" preserveAspectRatio="none" aria-label="Live S&P 500 proxy versus equal-weight performance from Nasdaq">
        ...
        {activeTab === "Oil System" && (
          <div className="domain-page tab-page">
            <header className="domain-hero oil-domain"><div><span>OIL TRANSMISSION SYSTEM</span><h2>Crude, products and physical access are different stories.</h2><p>War headlines, OPEC+ barrels, demand, stocks, refining margins and freight are separated so one signal cannot stand in for the whole oil system.</p></div><div className="domain-stat"><b>{market.cracks.length}</b><span>live crack proxies</span><small>Calculated from EIA daily spot prices</small></div></header>
            <section className="crack-grid">{market.cracks.map((crack) => <CrackCard key={crack.id} crack={crack} range={range} />)}</section>
            <div className="domain-layout oil-layout"><section className="thread-grid">{oilThreads.map((thread) => <article className="panel thread-card" key={thread.id}><header><span>{thread.category}</span><b>{thread.importance}</b></header><h3>{thread.headline}</h3><p>{thread.summary}</p><div className="thread-view"><small>CURRENT MARKET QUESTION</small><p>{thread.current_view || "Monitor the next primary release."}</p></div><footer><div>{thread.affected_assets.map((asset) => <span key={asset}>{asset}</span>)}</div><a href={thread.source_url} target="_blank" rel="noreferrer">Source ↗</a></footer></article>)}</section><aside className="panel domain-side"><PanelTitle icon="◉" title="Physical confirmation" /><MiniMarketChart series={marketBySymbol.get("CL=F")} range={range} title="WTI crude" /><MiniMarketChart series={marketBySymbol.get("HO=F")} range={range} title="ULSD / heating oil" /><a className="method-link" href="https://www.eia.gov/finance/markets/products/prices.php" target="_blank" rel="noreferrer">EIA crack-spread methodology ↗</a><p className="method-note">The app uses EIA spot-price proxies. They are useful for direction and comparison, but they are not a replacement for regional refinery margins or location-specific physical cracks.</p></aside></div>
          </div>
        )}
        ...
        {activeTab === "Charts" && (
          <div className="charts-page tab-page">
            <article className="panel primary-chart-panel">
              <div className="panel-title-row"><div><span className="panel-kicker">LIGHTWEIGHT LIVE CHART</span><h2>{selectedSeries?.label || "Market series unavailable"}</h2><p>{selectedSeries?.symbol || "—"} · last {formatValue(selectedSeries?.last)}</p></div><div className="range-tabs">{(["7D", "30D", "90D", "1Y"] as Range[]).map((item) => <button key={item} className={range === item ? "active" : ""} onClick={() => setRange(item)}>{item}</button>)}</div></div>
              <svg className="large-chart" viewBox="0 0 1000 430" preserveAspectRatio="none" aria-label={`Live ${selectedSeries?.label || "market"} chart from ${selectedSeries?.sourceName || "official market data"}`}>
              ...
              <div className="chart-footer"><span>{range}</span><span>5D {formatPct(selectedSeries?.change5d)}</span><span>21D {formatPct(selectedSeries?.change21d)}</span>{selectedSeries?.sourceUrl && <a href={selectedSeries.sourceUrl} target="_blank" rel="noreferrer">{selectedSeries.sourceName} ↗</a>}</div>
            </article>
...
function MiniMarketChart({ series, range, title, large = false }: { series?: MarketSeries; range: Range; title: string; large?: boolean }) {
  ...
  return <div className={`linked-market-chart ${large ? "large" : ""}`}><div><span>{title}</span><b>{series?.label || "Series unavailable"}</b><small>{series ? `${series.symbol} · 21D ${formatPct(series.change21d)}` : "Market data unavailable"}</small></div><svg viewBox="0 0 620 190" preserveAspectRatio="none"><path className="chart-grid" d="M0 48H620 M0 95H620 M0 142H620" />{points.length > 1 && <path className="chart-line" d={linePath(points,620,190,10)} />}</svg><footer><span>Last {formatValue(series?.last)}</span>{series?.sourceUrl && <a href={series.sourceUrl} target="_blank" rel="noreferrer">{series.sourceName} ↗</a>}</footer></div>;
}

function CrackCard({ crack, range }: { crack: CrackSeries; range: Range }) {
  const points = rangePoints(crack.points, range);
  return <article className="panel crack-card"><header><div><span>EIA SPOT-PRICE PROXY</span><h3>{crack.label}</h3></div><b>{formatValue(crack.last,1)}</b></header><svg viewBox="0 0 520 180" preserveAspectRatio="none"><path className="chart-grid" d="M0 45H520 M0 90H520 M0 135H520" />{points.length > 1 && <path className="chart-line" d={linePath(points,520,180,9)} />}</svg><div className="crack-metrics"><span>5D <b>{formatPct(crack.change5d)}</b></span><span>21D <b>{formatPct(crack.change21d)}</b></span></div><p>{crack.formula}</p><a href={crack.sourceUrl} target="_blank" rel="noreferrer">EIA source ↗</a></article>;
}
...
function MacroReleaseCard({ item }: { item: MacroRelease }) {
  return <article className={`panel macro-card ${item.category.toLowerCase()}`}><header><div><span>{item.release_name}</span><small>{item.agency} · {articleDate(item.release_date)} · {item.release_time_label}</small></div><b>{item.status}</b></header><h3>{item.reference_period || "Current release"}</h3><div className="macro-values"><div><small>ACTUAL</small><strong>{item.actual || "Awaiting"}</strong></div><div><small>CONSENSUS</small><strong>{item.consensus || "Not loaded"}</strong></div><div><small>PREVIOUS</small><strong>{item.revised_previous || item.previous || "Not loaded"}</strong></div></div><div className="macro-question"><small>DECIDING QUESTION</small><p>{item.watch_question}</p></div>{item.market_interpretation && <div className="macro-interpretation"><small>MARKET INTERPRETATION</small><p>{item.market_interpretation}</p></div>}<div className="macro-conditions"><div><small>CONFIRMATION</small><p>{item.confirmation_trigger || "Set after the release."}</p></div><div><small>INVALIDATION</small><p>{item.invalidation_trigger || "Set after the release."}</p></div></div><footer><div>{item.affected_assets.map((asset) => <span key={asset}>{asset}</span>)}</div><a href={item.source_url} target="_blank" rel="noreferrer">Official source ↗</a></footer></article>;
}
