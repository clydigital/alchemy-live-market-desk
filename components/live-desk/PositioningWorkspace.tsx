...
function scoreLabel(score: number, type: "commercial" | "spec") {
  if (type === "commercial") {
    if (score >= 81) return "Inverted extreme";
            ...
            <div className={styles.gridTwo}>
              <section className={styles.panel}>
                ...
                <div className={styles.extremes}>
                  ...
                  <article className={styles.extremeCard}><small>Commercial inverted extreme</small><b>{extremes.commercial.label}</b><strong className={styles.blue}>{extremes.commercial.commercialDisplayScore}</strong></article>
                  <article className={styles.extremeCard}><small>Largest weekly shift</small><b>{extremes.weekly.label}</b><strong className={styles.amber}>{signed(extremes.weekly.weeklyChange)}</strong></article>
                </div>
              </section>
          ...
          <section className={styles.panel}>
            <div className={styles.panelHead}>
              <div>
                ...
                <p>Commercials use an inverted display score. Raw Commercial percentiles remain available in the Alchemy View.</p>
              </div>
            ...
            <div className={styles.methodNote}>
              <strong>Commercial-score inversion</strong>
              <p>Commercial tiles display 100 minus the raw Commercial percentile for visual consistency. The raw score and raw net position remain preserved.</p>
            </div>
                ...
                {filtered.map((snapshot) => {
                  ...
                  return (
                    <div style={{ display: "contents" }} key={snapshot.code}>
                      ...
                      <div className={`${styles.tile} ${Math.abs(snapshot.weeklyChange) >= 10 ? styles.warm : styles.neutral}`}><strong>{signed(snapshot.weeklyChange)}</strong><span>Percentile points</span></div>
            ...
            <div className={styles.sourceLine}>
              <span>Scores use the latest observation against up to 52 prior weekly reports. Positioning provides context and does not time entries.</span>
              <a href={snapshots[0].sourceUrl} target="_blank" rel="noreferrer">Open official CFTC dataset ↗</a>
            </div>
          </section>
        </>
