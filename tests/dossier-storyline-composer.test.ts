...
test("edition composition supersedes rather than mutates the immutable base edition", () => {
  ...
  assert.match(edition, /edition_phase: "composed"/);
  assert.doesNotMatch(edition, /method:\s*"PATCH"[\s\S]{0,500}dossierComposition/);
  assert.match(migration, /drop index if exists public\.hybrid_daily_brief_run_unique/);
  assert.match(migration, /hybrid_daily_brief_run_phase_unique/);
  assert.match(migration, /research_run_id, edition_phase/);
});
